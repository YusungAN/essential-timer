...
function StatsViewer(props: {recordList: SolvedRecord[]}) {
    ...
    function calcBasicStats() {
        ...
        if (recordList.length >= 12) {
            for (let i = 0; i < recordList.length-11; i++) {
                let tempArr = recordList.slice(i, i+12).map((item) => item.penalty === '+2' ? item.record + 2 : (item.penalty === 'DNF' ? Infinity : item.record));
                tempArr.sort();
    ...
    return (
        <>
            <div>now/best Ao5: {nowAo5 !== null ? time2Str(nowAo5) : '---'}/{bestAo5 !== null ? time2Str(bestAo5) : '---'}</div>
            <div>now/best Ao12: {nowAo12 !== null ? time2Str(nowAo12) : '---'}/{bestAo12 !== null ? time2Str(bestAo12) : '---'}</div>
        </>
    );
}
